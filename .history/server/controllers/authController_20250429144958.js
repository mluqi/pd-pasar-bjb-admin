...
exports.signup = async (req, res) => {
  ...
  // Ensure your authMiddleware runs *before* this route handler
  if (!req.user || !req.user.id || !req.user.level) {
      // This should ideally be caught by the authMiddleware itself
      return res.status(401).json({ message: "Otentikasi diperlukan untuk tindakan ini." });
  }
  const creatorId = req.user.id;
  const creatorLevel = req.user.level; // e.g., 'SUA', 'PTG'
  ...
  try {
    ...
    const {
      ...
      user_phone,
      user_email,
      user_level_name, // Name of the level for the NEW user
      user_status,
      pasar_code // Sent ONLY if creator is Super Admin selecting a pasar
    } = req.body;

    // --- Basic Input Validation ---
    // Note: pasar_code is only required if creatorLevel is 'SUA'
    if (
      !user_name ||
      !user_pass ||
      !user_phone ||
    ...
    // --- Find Level Code for the NEW user ---
    ...
    // --- Determine user_owner based on CREATOR'S level ---
    let newUserOwnerCode = null; // This will hold the pasar_code for the new user

    if (creatorLevel === 'SUA') { // Assuming 'SUA' is the Super Admin level code
        // Super Admin MUST provide a pasar_code for the new user
        if (!pasar_code) {
            return res.status(400).json({
                message: "Kode Pasar (pasar_code) harus dipilih oleh Super Admin.",
            });
        }
        // Optional: Validate if the provided pasar_code exists
        ...
        newUserOwnerCode = pasar_code; // Assign the selected pasar code

    } else if (creatorLevel === 'PTG') { // Assuming 'PTG' is the Petugas level code
        // Petugas assigns the new user to their OWN pasar
        // Fetch the creator's (Petugas) record to get their user_owner
        const creatorUser = await userakses.findByPk(creatorId, {
            attributes: ['user_owner']
        });
        if (!creatorUser || !creatorUser.user_owner) {
            // This indicates a data problem or the Petugas isn't assigned a pasar
        ...
        newUserOwnerCode = creatorUser.user_owner; // Assign creator's pasar code

    } else {
        // Handle other roles if necessary - maybe they cannot create users?
    ...
    const newUser = await userakses.create({
      user_code: generatedUserCode, // Use the generated code
      user_name,
      user_pass: hashedPassword,
      user_phone,
      user_email,
      user_level: newUserLevelCode, // Level code of the NEW user
      user_foto: user_foto_filename,
      user_status,
      user_owner: newUserOwnerCode, // Assign the determined pasar code
    });
